...
const TaskDetail = (props) => {
  ...
  const workspaces_store = useSelector((state) => state.workspaces.workspaces);
  const dispatch = useDispatch();

  const title = id === null ? "New Task" : "Edit Task";
  ...
  const onBlured = async () => {
    ...
    if (currentTaskId === null) {
      formData.append("api_method", "add_task");
    ...
    try {
      ...
      if (response.data.success === 1) {
        if (currentTaskId === null) setCurrentTaskId(response.data.new_task_id);
        setDisabled(false);
      }
    } catch (err) {
      console.log(err);
  ...
  const onDeleteClicked = async () => {
    ...
    try {
      const response = await post(server_domain, formData);
      if (response.data.success === 1) {
        const newWorkspaces = workspaces_store.map((workspace) => {
        ...
        <div className="d-flex" style={{ height: "65%" }} id="div2">
          ...
          <div className="px-4 bg-white" style={{ width: "55%" }}>
            ...
            <Form.Group className="mb-3" controlId="taskDocuments">
              ...
              <DocumentsLinks disabled={disabled} taskId={currentTaskId} />
            </Form.Group>
            <Button
              variant="outline-danger"
              className="mb-3"
              onClick={onDeleteClicked}
              disabled={currentTaskId ? false : true}
              style={{ float: "right" }}
            >
              Delete
            </Button>
